...
export type CmdCallHttpAuthenticationActionReq = {
  authName: string;
  actionIndex: number;
...
export type CmdGetHttpAuthenticationConfigReq = {
  authName: string;
  values: { [key in string]?: JsonPrimitive };
  model: AnyModel;
  environmentId: string | null;
...
export type CmdGrpcGoReq = {
  requestId: string;
  environmentId: string | null;
  protoFiles: Array<string>;
};

export type CmdGrpcReflectReq = {
  requestId: string;
  environmentId: string | null;
  protoFiles: Array<string>;
};
...
export type CmdNewChildWindowReq = {
  url: string;
  label: string;
  title: string;
...
export type CmdRenderTemplateReq = {
  template: string;
  workspaceId: string;
  environmentId: string | null;
  purpose: RenderPurpose | null;
...
export type CmdSendEphemeralRequestReq = {
  request: HttpRequest;
  environmentId: string | null;
...
export type CmdSendHttpRequestReq = {
  environmentId: string | null;
  cookieJarId: string | null;
  requestId: string;
};
...
export type CmdTemplateFunctionConfigReq = {
  functionName: string;
  values: { [key in string]?: JsonPrimitive };
  model: AnyModel;
  environmentId: string | null;
...
export type ModelsUpsertGraphqlIntrospectionReq = {
  requestId: string;
  workspaceId: string;
  content: string | null;
};
...
export type RpcSchema = {
  ...
  cmd_send_feedback: [CmdSendFeedbackReq, null];
  cmd_dismiss_notification: [CmdDismissNotificationReq, null];
  cmd_grpc_reflect: [CmdGrpcReflectReq, Array<ServiceDefinition>];
  cmd_grpc_go: [CmdGrpcGoReq, string];
  cmd_restart: [CmdRestartReq, null];
  ...
  cmd_http_response_body_path: [CmdHttpResponseBodyPathReq, string | null];
  cmd_http_request_body: [CmdHttpRequestBodyReq, Array<number> | null];
  cmd_get_sse_events: [CmdGetSseEventsReq, Array<ServerSentEvent>];
  cmd_get_http_response_events: [CmdGetHttpResponseEventsReq, Array<HttpResponseEvent>];
  cmd_import_data: [CmdImportDataReq, BatchUpsertResult];
  cmd_import_url: [CmdImportUrlReq, BatchUpsertResult];
  cmd_http_request_actions: [CmdHttpRequestActionsReq, Array<GetHttpRequestActionsResponse>];
  cmd_websocket_request_actions: [
    CmdWebsocketRequestActionsReq,
    Array<GetWebsocketRequestActionsResponse>,
  ];
  cmd_call_websocket_request_action: [CmdCallWebsocketRequestActionReq, null];
  cmd_workspace_actions: [CmdWorkspaceActionsReq, Array<GetWorkspaceActionsResponse>];
  cmd_call_workspace_action: [CmdCallWorkspaceActionReq, null];
  cmd_folder_actions: [CmdFolderActionsReq, Array<GetFolderActionsResponse>];
  cmd_call_folder_action: [CmdCallFolderActionReq, null];
  cmd_grpc_request_actions: [CmdGrpcRequestActionsReq, Array<GetGrpcRequestActionsResponse>];
  cmd_template_function_summaries: [
    CmdTemplateFunctionSummariesReq,
    Array<GetTemplateFunctionSummaryResponse>,
  ];
  cmd_template_function_config: [CmdTemplateFunctionConfigReq, GetTemplateFunctionConfigResponse];
  cmd_get_http_authentication_summaries: [
    CmdGetHttpAuthenticationSummariesReq,
    Array<GetHttpAuthenticationSummaryResponse>,
  ];
  cmd_get_http_authentication_config: [
    CmdGetHttpAuthenticationConfigReq,
    GetHttpAuthenticationConfigResponse,
  ];
  cmd_call_http_request_action: [CmdCallHttpRequestActionReq, null];
  cmd_call_grpc_request_action: [CmdCallGrpcRequestActionReq, null];
  cmd_call_http_authentication_action: [CmdCallHttpAuthenticationActionReq, null];
  cmd_curl_to_request: [CmdCurlToRequestReq, HttpRequest];
  cmd_export_data: [CmdExportDataReq, null];
  cmd_save_base64_to_binary: [CmdSaveBase64ToBinaryReq, null];
  cmd_save_response: [CmdSaveResponseReq, null];
  cmd_send_http_request: [CmdSendHttpRequestReq, HttpResponse];
  cmd_reload_plugins: [CmdReloadPluginsReq, Array<[string, string]>];
  cmd_plugin_info: [CmdPluginInfoReq, PluginMetadata];
  cmd_delete_all_grpc_connections: [CmdDeleteAllGrpcConnectionsReq, null];
  cmd_delete_send_history: [CmdDeleteSendHistoryReq, null];
  cmd_delete_all_http_responses: [CmdDeleteAllHttpResponsesReq, null];
  cmd_get_workspace_meta: [CmdGetWorkspaceMetaReq, WorkspaceMeta];
  cmd_new_child_window: [CmdNewChildWindowReq, null];
  cmd_new_main_window: [CmdNewMainWindowReq, null];
  cmd_check_for_updates: [CmdCheckForUpdatesReq, boolean];
  cmd_decrypt_template: [CmdDecryptTemplateReq, string];
  cmd_secure_template: [CmdSecureTemplateReq, string];
  cmd_get_themes: [CmdGetThemesReq, Array<GetThemesResponse>];
  cmd_enable_encryption: [CmdEnableEncryptionReq, null];
  cmd_reveal_workspace_key: [CmdRevealWorkspaceKeyReq, string];
  cmd_set_workspace_key: [CmdSetWorkspaceKeyReq, null];
  cmd_disable_encryption: [CmdDisableEncryptionReq, null];
  cmd_default_headers: [CmdDefaultHeadersReq, Array<HttpRequestHeader>];
  models_upsert: [ModelsUpsertReq, string];
  models_delete: [ModelsDeleteReq, string];
  models_duplicate: [ModelsDuplicateReq, string];
  models_websocket_events: [ModelsWebsocketEventsReq, Array<WebsocketEvent>];
  models_grpc_events: [ModelsGrpcEventsReq, Array<GrpcEvent>];
  models_get_settings: [ModelsGetSettingsReq, Settings];
  models_get_graphql_introspection: [ModelsGetGraphqlIntrospectionReq, GraphQlIntrospection | null];
  ...
  cmd_git_branch: [CmdGitBranchReq, null];
  cmd_git_delete_branch: [CmdGitDeleteBranchReq, BranchDeleteResult];
  cmd_git_delete_remote_branch: [CmdGitDeleteRemoteBranchReq, null];
  cmd_git_merge_branch: [CmdGitMergeBranchReq, null];
  cmd_git_rename_branch: [CmdGitRenameBranchReq, null];
  ...
  cmd_git_log: [CmdGitLogReq, Array<GitCommit>];
  cmd_git_log_for_file: [CmdGitLogForFileReq, Array<GitCommit>];
  cmd_git_file_diff_for_commit: [CmdGitFileDiffForCommitReq, GitFileDiff];
  cmd_git_initialize: [CmdGitInitializeReq, null];
  cmd_git_clone: [CmdGitCloneReq, CloneResult];
  cmd_git_commit: [CmdGitCommitReq, null];
  cmd_git_fetch_all: [CmdGitFetchAllReq, null];
  ...
  cmd_git_add: [CmdGitAddReq, null];
  cmd_git_unstage: [CmdGitUnstageReq, null];
  cmd_git_reset_changes: [CmdGitResetChangesReq, null];
  cmd_git_restore_files: [CmdGitRestoreFilesReq, null];
  cmd_git_restore_file_from_commit: [CmdGitRestoreFileFromCommitReq, null];
  cmd_git_add_credential: [CmdGitAddCredentialReq, null];
  cmd_git_remotes: [CmdGitRemotesReq, Array<GitRemote>];
  cmd_git_add_remote: [CmdGitAddRemoteReq, GitRemote];
  cmd_git_rm_remote: [CmdGitRmRemoteReq, null];
  cmd_sync_calculate: [CmdSyncCalculateReq, Array<SyncOp>];
  cmd_sync_calculate_fs: [CmdSyncCalculateFsReq, Array<SyncOp>];
  cmd_sync_apply: [CmdSyncApplyReq, null];
  cmd_ws_delete_connections: [CmdWsDeleteConnectionsReq, null];
  ...
  cmd_plugins_install: [CmdPluginsInstallReq, null];
  ...
  cmd_plugin_init_errors: [CmdPluginInitErrorsReq, Array<[string, string]>];
  cmd_plugins_updates: [CmdPluginsUpdatesReq, PluginUpdatesResponse];
  cmd_plugins_update_all: [CmdPluginsUpdateAllReq, Array<PluginNameVersion>];
